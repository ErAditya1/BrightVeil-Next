...
function FileList({ items, onRender, onEdit }: ChapterListProps) {
    const [isMounted, setIsMounted] = useState(false)
    const {course_id} = useParams()
    
    const [file, setfile] = useState(items)
    useEffect(() => {
        setfile(items)
    }, [items])
    useEffect(() => {
    ...
    const onDragEnd = (result: DropResult) => {
        ...
        const updatedfile = items.slice(startIndex, endIndex+1)

        // setfile(items)


        const bulkItems = updatedfile.map((chapter) => ({ 
            id: chapter._id, 
