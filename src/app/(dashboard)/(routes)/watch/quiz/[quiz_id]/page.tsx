    ...
    return (
      ...
      {loading ? (
        ...
        >
          ...
          {score !== null && (
            <motion.div
              className="mt-6 text-center text-xl font-semibold text-gray-800 dark:text-gray-100"
              initial={{ opacity: 0, y: 50 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5 }}
            >
              <p>Your Score: <span className="text-blue-500">{score}</span> / {questions?.length}</p>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                Great job! You can always retry the quiz to improve your score.
              </p>
            </motion.div>
          )}
        </motion.div>
